...
if run_delete_requests:
    r = s.delete("http://wulf.local/dcc/locos/3")
...
"""
if run_get_requests:
    r = s.get("http://wulf.local/dcc/locos/")
    print(str(r.request) + " " + str(r.url) + " " + str(r.status_code))
    if len(r.content) and r.headers.get("content-type") == "application/json":
        print(r.json())

    r = s.get("http://wulf.local/dcc/locos/3")
    print(str(r.request) + " " + str(r.url) + " " + str(r.status_code))
    if len(r.content) and r.headers.get("content-type") == "application/json":
        print(r.json())
...
"""
if run_put_requests:
    r = s.put("http://wulf.local/dcc/locos/3", data=valid)
    print(str(r.request) + " " + str(r.url) + " " + str(r.status_code))

    # Expect 415 (invalid json)
    r = s.put("http://wulf.local/dcc/locos/3", data=invalid)
    print(str(r.request) + " " + str(r.url) + " " + str(r.status_code))

    # Expect 411 (no content)
    r = s.put("http://wulf.local/dcc/locos/3")
    print(str(r.request) + " " + str(r.url) + " " + str(r.status_code))
